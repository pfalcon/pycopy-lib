...
from test.support import run_unittest, cpython_only #, captured_stdout
import io
import locale
import re
    ...
    def checkPatternError(self, pattern, errmsg, pos=None):
        ...
        with self.subTest(pattern=pattern):
            ...
            if errmsg is not None:
                ...
                if err.msg != errmsg:
                    print("Warning: error message doesn't match: %r vs %r" % (err.msg, errmsg))
            if pos is not None:
                #self.assertEqual(err.pos, pos)
                if err.pos != pos:
                    print("Warning: error pos doesn't match: %r vs %r" % (err.pos, pos))
...
#                       memoryview(b"a:b::c:::d")):
    ...
    @unittest.skip("No sre_constants.MAXGROUPS")
...
#        self.checkPatternError(r'(abc\1)', 'cannot refer to an open group', 4)
    ...
    @unittest.skip("No .groupdict()")
    def test_groupdict(self):
        self.assertEqual(re.match('(?P<first>first) (?P<second>second)',
                                  'first second').groupdict(),
                         {'first':'first', 'second':'second'})

    @unittest.skip("No Match.expand()")
    ...
    @unittest.skip("No attrs on Match")
...
#        self.assertEqual(re.match(r, "\uff01").group(), "\uff01")
    ...
    def test_lookbehind(self):
        ...
        # Group reference.
# pcre: "lookbehind assertion is not fixed length". Apparently can be worked around in pcre2
...
#        self.checkPatternError(r"[\w-b]", r'bad character range \w-b', 1)
    ...
    @unittest.skip("Scanner is non-public")
...
#        self.assertIsNone(scanner.search())
    ...
    @unittest.skip("array is not supported")
...
#        self.assertTrue(re.match('(?ixu) ' + lower_char, upper_char))
    ...
    @unittest.skip("In Pycopy, mixing is allowed")
...
#        self.assertRaises(ValueError, re.compile, '(?au)\w')
    ...
    @unittest.skip("LOCALE isn't really implemented")
...
#    @bigmemtest(size=_2G, memuse=1)
    def _test_large_search(self, size):
...
#    @bigmemtest(size=_2G, memuse=16 + 2)
    def _test_large_subn(self, size):
        # Issue #10182: indices were 32-bit-truncated.
        s = 'a' * size
        r, n = re.subn('', '', s)
...
#        self.assertRaises(OverflowError, re.compile, r".{%d,%d}" % (2**129, 2**128))
    ...
    @unittest.skip("repr is currently incompatible")
    ...
    @unittest.skip("re.DEBUG not implemented")
...
'''
    ...
    @unittest.skip("passing string as kwarg is currently not supported")
    ...
    @unittest.skip("re.error is not fully compliant")
    def test_error(self):
        with self.assertRaises(re.error) as cm:
            re.compile('(\u20ac))')
        err = cm.exception
...
@unittest.skip("repr is not supported")
